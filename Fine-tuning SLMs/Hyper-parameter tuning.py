...
model_name = "dmis-lab/biobert-v1.1"  # You can replace this with any pre-trained model
labels = 2  # Number of target labels (binary classification in this case)
metric_to_use = 'f1'  # Metric to optimize during hyperparameter tuning
...
def tokenize_function(examples):
    ...
    stride = max_length // 2  # Overlapping tokens between segments
    input_ids_list = []
    attention_mask_list = []
    labels_list = []
...
training_args = TrainingArguments(
    output_dir='./OUTPUT/testing/',  # Directory to save the model outputs
    overwrite_output_dir=True,  # Overwrite the content of the output directory
    do_train=True,
    do_eval=True,
    num_train_epochs=1,  # Number of training epochs (could be set higher)
